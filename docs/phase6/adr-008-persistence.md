...
During our manual Disaster Recovery Drill, we identified a conflict between AOF and our automated RDB backups. If `--appendonly yes` is active, Redis prioritizes the AOF log over any injected `dump.rdb` file during a pod restart, causing the restore to fail (`nil` output).

**Mitigation:** For this environment, we disabled AOF (`--appendonly no`). We rely on daily RDB snapshots generated by a CronJob. This raises the RPO to 24 hours, but it improves the RTO because recovery becomes a straightforward file replacement procedure.
